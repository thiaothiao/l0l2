...
Lagrangian dual of $(\text{DCR}^{\rho, \mu})$ is 
\[
		(\text{DUAL}^{\rho, \mu})\;\left\{\begin{array}{ll}
\max & G(\nu):=-\ddfrac{1}{4}\left\|\nu\right\|^2_2 - b^T\nu - \rho(\left\|.\right\|_0 + \mu^2 \left\|.\right\|_2^2)^{*}(-\ddfrac{A^T\nu}{\rho})\\
		s.t.&\;\nu\in\mathbb{R}^m.
		\end{array} \right.
		\]
With 
		\[
			(\left\|.\right\|_0 + \mu^2 \left\|.\right\|_2^2)^{*}(z) = \sum_{i}^{m}(\left|.\right|_0 + \mu^2 \left|.\right|_2^2)^{*}(z_i),
		\]
\[
			(\left|.\right|_0 + \mu^2 \left|.\right|_2^2)^{*}(z_i) = (\ddfrac{1}{4\mu^2}z_i^2 - 1)^+=\left\{ \begin{array}{ll}
\ddfrac{1}{4\mu^2}z_i^2 - 1 &\;\text{if}\; \left|z_i\right|\geq 2\mu\\
0& \text{otherwise}.
\end{array}
\right.
		\]

Weak duality holds. As Slater's conditition is satisfied, strong duality holds for $(\text{DCR}^{\rho, \mu})$.\\
This formulation will help to find a dual feasible efficiently!
\subsubsection{Optimality condition for $(\text{DUAL}^{\rho, \mu})$}
The optimality condition for the convex program $(\text{DUAL}^{\rho, \mu})$ is given by,
\[
	\ddfrac{1}{2}\nu+b\in A\partial (\left\|.\right\|_0 + \mu^2 \left\|.\right\|_2^2)^{*}(-\ddfrac{A^T\nu}{\rho}).
\]
As
\[
\partial (\left\|.\right\|_0 + \mu^2 \left\|.\right\|_2^2)^{*}(z_i)=\left\{ \begin{array}{ll}
\left\{\ddfrac{1}{2\mu^2}z_i\right\}&\;\text{if}\; \left|z_i\right|> 2\mu\\
\left\{0\right\} &\;\text{if}\; \left|z_i\right|< 2\mu\\
\left[0, \ddfrac{1}{\mu}\right]&\;\text{if}\; z_i= 2\mu,\\
\left[-\ddfrac{1}{\mu}, 0\right]&\;\text{if}\; z_i= -2\mu,
\end{array}
\right.
\]
the condition becomes
\[
\left(\underset{opt}{\text{DUAL}^{\rho, \mu}}\right)\;
\ddfrac{1}{2}\nu+b = Ay,\; 
y_i\in 
\left\{ \begin{array}{ll}
\left\{-\ddfrac{1}{2\rho\mu^2}\left[A^T\nu\right]_i\right\}&\;\text{if}\; \left|\left[A^T\nu\right]_i\right|> 2\rho\mu\\
\left\{0\right\} &\;\text{if}\; \left|\left[A^T\nu\right]_i\right|< 2\rho\mu\\
\left[0, \ddfrac{1}{\mu}\right]&\;\text{if}\; \left[A^T\nu\right]_i= -2\rho\mu,\\
\left[-\ddfrac{1}{\mu}, 0\right]&\;\text{if}\; \left[A^T\nu\right]_i= 2\rho\mu.
\end{array}
\right.
\]
...
\[
		\left\{\begin{array}{ll}
\max & -\ddfrac{1}{4}\left\|\nu_0\right\|^2_2 s^2 - b^T\nu_0 s - \rho(\left\|.\right\|_0 + \mu^2 \left\|.\right\|_2^2)^{*}(-\ddfrac{A^T\nu_0}{\rho}s)\\
		s.t.&\;s\in\mathbb{R},
		\end{array} \right.
		\]
say 
\[
		\left\{\begin{array}{ll}
\max & -\ddfrac{1}{4}\left\|\nu_0\right\|^2_2 s^2 - b^T\nu_0 s - \rho\sum_{i=1}^{n} (\ddfrac{\left[A^T\nu_0\right]_i^2}{4\mu^2\rho^2}s^2 - 1)^+\\
		s.t.&\;s\in\mathbb{R}.
		\end{array} \right.
		\]
...
\[
		\left\{\begin{array}{ll}
\max & -\ddfrac{1}{4}\left\|\nu_0\right\|^2_2 s^2 + \left|b^T\nu_0\right| s - \rho\sum_{i=1}^{n} (\ddfrac{\left[A^T\nu_0\right]_i^2}{4\mu^2\rho^2}s^2 - 1)^+\\
		s.t.&\;s\geq0,
		\end{array} \right.
		\]
and then update the sign of $s$.\\
Define 
\[\theta_i= \ddfrac{\left|\left[A^T\nu_0\right]_i\right|}{2\mu\rho} .\]
Let us consider the set with distinct and ordered elements $\gamma_j$
\[
		\left\{\ddfrac{1}{\theta_i}:\;\theta_i\neq 0,\;i=1,...,n \right\} \cup  \left\{0,+\infty \right\}= \left\{\gamma_j:\;j=1,...,p \right\}.
\]
\[
		\left\{\begin{array}{ll}
\max & -\ddfrac{1}{4}\left\|\nu_0\right\|^2_2 s^2 + \left|b^T\nu_0\right| s - \rho\sum_{i=1,\theta_i\neq 0, \ddfrac{1}{\theta_i}\leq\gamma_j}^{n} \theta_i^2 (s^2 - \ddfrac{1}{\theta_i^2 })^+\\
		s.t.&\;s\in\left[\gamma_j, \gamma_{j+1}\right],
		\end{array} \right.
		\]
\[
		\left\{\begin{array}{ll}
\max & -\ddfrac{1}{4}\left\|\nu_0\right\|^2_2 s^2 + \left|b^T\nu_0\right| s - \rho\sum_{i=1,\theta_i\neq 0, \ddfrac{1}{\theta_i}\leq\gamma_j}^{n} \theta_i^2 (s^2 - \ddfrac{1}{\theta_i^2 })\\
		...
		\end{array} \right.
		\]
So we will solve the following sequence of 1D convex quadratic programs
\[
\left\{
\begin{array}{ll}		
\max & -\left[\ddfrac{1}{4}\left\|\nu_0\right\|^2_2 + \rho\sum_{i=1,\theta_i\neq 0, \ddfrac{1}{\theta_i}\leq\gamma_j}^{n} \theta_i^2\right] s^2 + \left|b^T\nu_0\right| s + \rho\sum_{i=1,\theta_i\neq 0, \ddfrac{1}{\theta_i}\leq\gamma_j}^{n} 1\\
		s.t.&\;s\in\left[\gamma_j, \gamma_{j+1}\right].
		\end{array} \right.
		\]
\[
		a<0, \;\left\{\max a s^2 +b s+c:\;s\in\left[\gamma_j, \gamma_{j+1}\right]\right\}\leftrightarrow
s=\left\{
\begin{array}{ll}
\gamma_j& \text{if}\;-\ddfrac{b}{2a} < \gamma_j,\\
\gamma_{j+1}& \text{if}\;-\ddfrac{b}{2a} > \gamma_{j+1},\\
-\ddfrac{b}{2a}& \text{otherwise}.
\end{array}
 \right.
\]
The sequence is solved efficiently using it's incremental nature.
...
\begin{proof} ...
\end{proof}
\subsection{Cyclical coordinate descent for solving $(\mathbb{Q}^{\beta,\delta})$}
\begin{algorithm}[H]
        \caption{A cyclical coordinate descent for $(\mathbb{Q}^{\beta,\delta})$}
        \label{alg:cda}
...
\ddfrac{\alpha_j - \delta\beta\text{sign}(\alpha_j)}{ z_j} & \delta\beta <\left|\alpha_j\right|\leq \delta\beta +\delta z_j,\\
\ddfrac{\alpha_j}{\beta + z_j} &\delta\beta +\delta z_j \leq \left|\alpha_j\right|.
\end{array}
\right.
\]
           	 \EndFor
...
\[
	\left\{\begin{array}{ll}
\min & \left\|Ax-b\right\|^2_2 + \beta\delta\left\| s + \delta v\right\|_2^2 - 2\beta\delta^2 e^Tv\\
...
\subsection{Entire path direct solutions}
This is a first approach that allow entire path solutions computation using Gauss eliminations. LARS like algorithm is also under progress!
\subsubsection{Computing a new solution from a known one}
