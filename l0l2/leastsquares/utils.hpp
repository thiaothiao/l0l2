...
namespace l0l2
{
    ...
    template <std::floating_point ScalarType> class Utils final
    {
      public:
        using Scalar = ScalarType;

        static constexpr Scalar epsilon = std::is_same_v<Scalar, float>
                                              ? static_cast<Scalar>(1e-6)
                                              : static_cast<Scalar>(1e-8);

        static Scalar sign(Scalar value);

        static Scalar solveMaxConcaveQP1D(Scalar a, Scalar b, Scalar c,
                                          Scalar s0, Scalar s1);
    ...
    {
        return std::signbit(value) ? static_cast<Scalar>(-1)
                                   : static_cast<Scalar>(1);
    }

    template <std::floating_point ScalarType>
    Utils<ScalarType>::Scalar
    Utils<ScalarType>::solveMaxConcaveQP1D(Scalar a, Scalar b, Scalar c,
                                           Scalar s0, Scalar s1)
    {
    ...
    namespace linearmodel
    {
        namespace leastsquares
        {
            ...
            using CoordinateStates =
                Eigen::Array<CoordinateState, Eigen::Dynamic, 1>;
            ...
            template <std::floating_point ScalarType> struct Solution
            {
                ...
                Solution(Scalar deltaInput = std::numeric_limits<Scalar>::max(),
                    ...
                    : delta{deltaInput}, x{xInput},
                      intercept{static_cast<Scalar>(0)},
                      dualityGap{static_cast<Scalar>(0)}, grad{gradInput}
                {
                }

                Solution(Vector<Scalar> &&xInput,
                         Scalar interceptInput = static_cast<Scalar>(0))
